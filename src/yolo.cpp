...
bool Yolo::Detect(Mat& SrcImg, Net& net, vector<Output>& output) {
    ...
    // blobFromImage(netInputImg, blob, 1 / 255.0, cv::Size(netWidth, netHeight),
    // cv::Scalar(104, 117, 123), true, false); blobFromImage(netInputImg, blob, 1
    // / 255.0, cv::Size(netWidth, netHeight), cv::Scalar(114, 114,114), true,
    // false);
    net.setInput(blob);
                ...
                for (int j = 0; j < grid_x; j++) {
                    ...
                    if (box_score >= boxThreshold) {
                        ...
                        if (max_class_socre >= classThreshold) {
                            ...
                            boxes.push_back(
                                Rect(left, top, int(w * ratio_w), int(h * ratio_h)));
                        }
                    }
                    pdata += net_width; //下一行
                }
...
cv::Mat Yolo::drawPred(Mat& img, vector<Output> result, vector<Scalar> color) {
    for (size_t i = 0; i < result.size(); i++) {
        ...
        top = result[i].box.y;
//        int color_num = i;
//        if (className[result[i].id] != "person" and className[result[i].id] != "car") continue;
...
//        // 显示类别
//        string label =
//            className[result[i].id] + ":" + to_string(result[i].confidence);
//        int baseLine;
//        Size labelSize =
...
//        // 显示logo
//        Mat imageROI;
//        string img_path = "../image/" + className[result[i].id] + ".png";
//        Mat logo = imread(img_path);
//        if (logo.empty()) {
//            LOG_F(INFO, "%s is empty", img_path.c_str());
//        }
//        // LOG_F(INFO, "%d,  %d,  %d,  %d", left + int(result[i].box.width * 0.5), top,
//        //       logo.cols, logo.rows);
//        Rect rect = Rect(0, 0, img.cols, img.rows);
//        if (isInside(Rect(left + int(result[i].box.width * 0.5), top - logo.rows, logo.cols, logo.rows), rect)) {
//            imageROI = img(Rect(left + int(result[i].box.width * 0.5), top - logo.rows, logo.cols, logo.rows));
