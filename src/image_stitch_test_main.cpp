...
class Quadrilateral{
public:
    Quadrilateral(std::vector<cv::Point> point_vec)
    : point(4){
        for(int i = 0;i < point_vec.size();i++) point[i] = point_vec[i];
    }

    void draw_on(cv::Mat image){
        for(int i = 0;i < 4;i++)
            cv::line(image, point[i], point[(i + 1) % 4], cv::Scalar(255,255,255), 4);
    }
...
int main(){
    ...
    std::filesystem::path parameters_dir("/home/xcmg/stitcher/360-stitcher/parameters");
...
//    yolo_detect::YoloDetect yolo_detect(weight_file_path, true);
    ...
    int HEIGHT = 2160;
    int WIDTH = 3840;
//    std::filesystem::path yamls_dir("/home/touch/data/360/parameters/yamls");
...
//        cv::imshow(std::to_string(camera_idx_vec[i]), undistorted_image_vec[i]);
//    }
//    cv::waitKey(0);
//    cv::Size undistorted_image_size = undistorter_vec[0].getNewImageSize();
//    int src_height = undistorted_image_size.height, src_width = undistorted_image_size.width;
...
//        undistorter_vec[i].getMask(mask_image);
        ...
        if (mask_image.rows != src_height) {
            ...
            std::cout << "resize " << camera_idx_vec[i] << " " << " scale " << scale << " " << camera_idx_vec[i] << std::endl;
...
//    cv::waitKey(0);
    ...
    std::filesystem::path image_dir("/home/xcmg/stitcher/data/922/camera_calib");
...
//            printf("%d need resize\n", camera_idx_vec[i]);
            cv::resize(undistorted_image_vec[i], undistorted_image_vec[i], cv::Size(src_width, src_height));
...
//    cv::waitKey(0);
    ...
    video_writer.open("./video/video_intersect.avi", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), 20,
...
//        cur_step.y += rand()%max_rand - max_rand / 2;
        ...
        if(Intersector::intersectRects(quadrilateral.convertToFloat3(), car_rect)){
