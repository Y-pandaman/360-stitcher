...
template<typename T, int CHANNEL>
static inline __device__ void Bilinear(
    T* src, T* dst,
    int h, int w, float2 pixel)
{
...
template<typename T, int CHANNEL>
__device__  static inline void weight_product(float *color, T *src, float &aw, float w)
{   
    #pragma unroll
    for (int i=0; i<CHANNEL; i++)
    {
    ...
    int height, int width)
{
    ...
    while(pixelIdx < totalPixel)
    {
        ...
        Bilinear<T, CHANNEL>(src, dst+pixelIdx*CHANNEL, src_h, src_w, grid[pixelIdx]);
    ...
    while(pixelIdx < totalPixel)
    {
        ...
        for (int i=0; i<5; i++)
        {
            ...
            for (int j=0; j<5; j++)
            {
                ...
                weight_product<short, CHANNEL>(color, src+CHANNEL*(v*2*width+u), weight, GauKernel[i*5+j]);
    ...
    int height, int width)
{
   ...
   while(pixelIdx < totalPixel)
   {
       ...
       float3 c1 = make_float3((float)rgb1.x, (float)rgb1.y, (float)rgb1.z) / MASK_MAX;
       float3 c2 = make_float3((float)rgb2.x, (float)rgb2.y, (float)rgb2.z) / MASK_MAX;
       float3 d = fabs(c1-c2);

       diff[pixelIdx] = (ushort)((d.x + d.y + d.z) / 3.0f * 65535);
...
__global__
void update_diff_use_seam_kernel(
        ushort* input_output_diff,
        uchar* seam,
        ushort delta_on_seam,
        int height, int width
        )
{
    int pixelIdx = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    while(pixelIdx < totalPixel)
    {
        if(seam[pixelIdx] != 255 && input_output_diff[pixelIdx] != 65535) input_output_diff[pixelIdx] = 0;
        pixelIdx += total_thread;
    }
}

__global__
void dilateDiff_kernel(
        ushort* diff_input,
        ushort* diff_output,
        int height, int width,
        int dilate_length,
        ushort threshold)
{
    ...
    while(pixelIdx < totalPixel)
    {
        diff_output[pixelIdx] = diff_input[pixelIdx];
        int pixel_col = pixelIdx % width;
        int pixel_row = pixelIdx / width;
        bool flag_dilated = false;
        for(int i = -dilate_length;i < dilate_length;i++){
            if(flag_dilated) break;
            for(int j = -dilate_length;j < dilate_length;j++){
                if(flag_dilated) break;

                int cur_pixel_col = pixel_col + j;
                if(cur_pixel_col < 0 || cur_pixel_col >= width) continue;
                int cur_pixel_row = pixel_row + i;
                if(cur_pixel_row < 0 || cur_pixel_row >= height) continue;

                int cur_pixel_idx = cur_pixel_row * width + cur_pixel_col;
//                if(diff_input[cur_pixel_idx] > threshold)
                if(diff_input[cur_pixel_idx] > threshold && diff_input[cur_pixel_idx] < 65535)
                {
                    diff_output[pixelIdx] = 65535;
                    flag_dilated = true;
                    break;
                }
            }
        }
        if(!flag_dilated && diff_output[pixelIdx] != 65535) diff_output[pixelIdx] /= 10;
        pixelIdx += total_thread;
    }
}
...
__global__ 
void diff_person_kernel(
    ushort* diff,
    float2* grid, int scale, 
    ...
    int height, int width)
{
   ...
   while(pixelIdx < totalPixel)
   {
       ...
       if (uv.x >= x_min && uv.x <= x_max && uv.y >= y_min && uv.y <= y_max)
       {
        ...
        //    printf("bbox %f %f %f %f \t uv %f %f\n", x_min, y_min, x_max, y_max, uv.x, uv.y);
           diff[pixelIdx] = 65535;
       }
...
__global__
void draw_car_kernel(
    uchar3* img,
    uchar4* icon,
    int icon_h, int icon_w,
    int height, int width)
{
   int pixelIdx = threadIdx.x + blockIdx.x * blockDim.x;
   int total_thread = blockDim.x * gridDim.x;
    ...
    int height, int width)
{
   ...
   while(pixelIdx < totalPixel)
   {
       ...
       for (int i=0; i<4; i++)
       {
           ...
           if (dis > thickness) continue;

           float3 La = make_float3(
               -1.0f * l.y,
               l.x,
               l.y * a.x - l.x * a.y);
           float3 Lb = make_float3(
               -1.0f * l.y,
               l.x,
               l.y * b.x - l.x * b.y);
...
void grid_sample(float2* grid, uchar* src, uchar* dst, int src_h, int src_w, int height, int width)
{
...
void grid_sample(float2* grid, uchar3* src, uchar3* dst, int src_h, int src_w, int height, int width)
{
...
void pyramid_downsample(CylinderImageGPU img, int scale, uchar3* scaled_img)
{
    ...
    short3 *current_img;
    checkCudaErrors(cudaMalloc((void**)&current_img, sizeof(short3)*height*width));
    ...
    for (int i=1; i<levels; i++)
    {
        ...
        checkCudaErrors(cudaMalloc((void**)&_down_scale_img, sizeof(short3)*height*width));
...
void compute_diff(uchar3 *img1, uchar3 *img2, ushort *&output_diff, ushort *&temp_diff, int height, int width)
{
    ...
    ushort *tp = output_diff;
    output_diff = temp_diff;
    temp_diff = tp;

    dilateDiff_kernel<<<NUM_BLOCK(height*width), NUM_THREAD>>>(
            temp_diff,
            output_diff,
            height, width,
            15, 65535 / 4
...
void update_diff_in_person_area(ushort* diff, float2* grid, int scale, float4 bbox, int height, int width)
{
...
void draw_circle(uchar3* img, float2 center, float radius, int thickness, int height, int width)
{
...
void draw_bbox(uchar3* img, float3* data, float2* grid, uchar* seam_mask, int thickness, int height, int width)
{
...
void draw_icon(uchar3* img, uchar4* icon, int icon_h, int icon_w, int height, int width)
{
...
void update_diff_use_seam(ushort *input_output_diff, cv::Mat prev_seam_map, int height, int width) {
    uchar *prev_seam;
    checkCudaErrors(cudaMalloc((void **) &prev_seam, sizeof(uchar) * height * width));
    checkCudaErrors(cudaMemcpy(prev_seam, prev_seam_map.ptr<uchar>(0),
            sizeof(uchar) * height * width, cudaMemcpyHostToDevice));
