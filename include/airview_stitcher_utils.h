...
static inline 
float3 homographBased_warp(float3 p, cv::Mat H)
{
    float3 warped_p;
    warped_p.x = H.at<float>(0,0) * p.x + H.at<float>(0,1) * p.y + H.at<float>(0,2) * p.z;
    warped_p.y = H.at<float>(1,0) * p.x + H.at<float>(1,1) * p.y + H.at<float>(1,2) * p.z;
    warped_p.z = H.at<float>(2,0) * p.x + H.at<float>(2,1) * p.y + H.at<float>(2,2) * p.z;

    return warped_p;
}

static inline 
bool is_boundary(cv::Mat src, int i, int j)
{
    if (i == 0 || j == 0 || i == src.rows-1 || j == src.cols-1) return true;
    for (int k=0; k<8; k++)
    {
...
static inline 
int count_mask(cv::Mat mask, int thresh=128)
{
    int count = 0;
    for (int i=0; i<mask.rows; i++)
    {
        ...
        {
            if (mask.at<uchar>(i,j) > thresh) count++;
        }
    }
    return count;
}
...
static inline 
bool get_overlapMask(std::vector<cv::Mat> &masks, std::vector<cv::Mat> &overlap_masks)
{
...
static inline
bool adjust_overlapmask(cv::Mat &a, cv::Mat &b)
{
    ...
    if (!is_overlap) return is_overlap;
    ...
    for (int i=0; i<a.rows; i++)
    {
        ...
        {
            if (a.at<uchar>(i,j) > 128) count_a++;
            if (b.at<uchar>(i,j) > 128) count_b++;
        }
    }
...
static inline 
void DFS_collect_overlap_point(
    int2 loc, int mark_number,
    cv::Mat marks,
    ...
    cv::Mat &visited)
{
    ...
    for (int k=0; k<8; k++)
    {
        ...
        if (next_loc.x < 0 || next_loc.y < 0 || 
            next_loc.x >= visited.rows || next_loc.y >= visited.cols) continue;

        if (visited.at<uchar>(next_loc.x, next_loc.y) == 1) continue;

        if (marks.at<uchar>(next_loc.x, next_loc.y) != mark_number) continue;

        DFS_collect_overlap_point(next_loc, mark_number, marks, overlap_points, visited);
    }
}

static inline 
int4 decide_start_end(cv::Mat overlap_mask, cv::Mat mask0, cv::Mat mask1)
{
    // STEP1 标记 oevrlap的边缘区域 
    cv::Mat marks = cv::Mat::zeros(overlap_mask.rows, overlap_mask.cols, CV_8UC1);
    for (int i=0; i<overlap_mask.rows; i++)
    {
        for (int j=0; j<overlap_mask.cols; j++)
        {

            if (overlap_mask.at<uchar>(i,j) != 255) continue;

            if (is_boundary(overlap_mask, i, j) && 
                is_boundary(mask0, i, j) &&
    ...
    cv::Mat visited = cv::Mat::zeros(overlap_mask.rows, overlap_mask.cols, CV_8UC1);
    std::vector<std::vector<int2>> overlap_points;
    for (int i=0; i<marks.rows; i++)
    {
        for (int j=0; j<marks.cols; j++)
        {
            if (marks.at<uchar>(i,j) != 255) continue;
            if (visited.at<uchar>(i,j) == 1) continue;

            std::vector<int2> temp;
            DFS_collect_overlap_point(make_int2(i,j), 255, marks, temp, visited);
    ...
    int d1 = (start.x - center.x) * (start.x - center.x)  + (start.y - center.y) * (start.y - center.y);
    int d2 = (end.x - center.x) * (end.x - center.x)  + (end.y - center.y) * (end.y - center.y);
    if (d1 > d2)
    {
...
//        printf("start at (%d, %d), current loc: (%d, %d)\n", start.x, start.y, loc.x, loc.y);


        for (auto k : search_seq)
        {
            int2 next_loc = loc + k;

            if (next_loc.x < 0 || next_loc.y < 0 || 
                next_loc.x >= diff.rows || next_loc.y >= diff.cols) continue;
            
            if (overlap_mask.at<uchar>(next_loc.x, next_loc.y) == 0) continue;

            if (visited.at<uchar>(next_loc.x, next_loc.y) == 1) continue;

            float w = ((float)diff.at<ushort>(next_loc.x, next_loc.y) / 65535.0f);

            if (D.at<float>(next_loc.x, next_loc.y) > D.at<float>(loc.x, loc.y) + w)
            {
                D.at<float>(next_loc.x, next_loc.y) = D.at<float>(loc.x, loc.y) + w;
                Q.emplace(D.at<float>(next_loc.x, next_loc.y), next_loc);

                path.at<float2>(next_loc.x, next_loc.y) = make_float2(loc.x,loc.y);
...
//                visited.at<uchar>(next_loc.x, next_loc.y) = 1;
    ...
    for (int i=0; i<overlap_mask.rows; i++)
    {
        ...
        if (overlap_mask.at<uchar>(i,overlap_mask.cols-1) != 0 && min_dis > D.at<float>(i,overlap_mask.cols-1))
        {
    ...
    for (int i=0; i<overlap_mask.cols; i++)
    {
        ...
        if (overlap_mask.at<uchar>(overlap_mask.rows-1, i) != 0 && min_dis > D.at<float>(overlap_mask.rows-1, i))
        {
    ...
    while (true)
    {
        ...
        if (end_pts.x == start.x && end_pts.y == start.y) break;
...
static inline 
void seam_search(int4 endPts, cv::Mat overlap_mask, cv::Mat diff, 
                 cv::Mat &seam_map, cv::Mat &seam_line, int seam_mark)
{
    int2 start = make_int2(endPts.x, endPts.y);
    int2 end = make_int2(endPts.z, endPts.w);

    Dijkstra_search_seam(start, end, diff, overlap_mask, seam_map, seam_line, seam_mark);
}


static inline 
void BFS_get_seam_mask(
    int2 start, 
    ...
    cv::Mat &visited)
{
    ...
    while(!Q.empty())
    {
        ...
        for (int k=0; k<4; k++)
        {
            ...
            // 需要在total mask内部
            if (total_mask.at<uchar>(next_loc.x,next_loc.y) < 128) continue;
...
static inline 
std::vector<cv::Mat> gen_seam_mask(
    cv::Mat total_mask,
    ...
    int num_view)
{
    ...
    // {
    //     seam_masks[i] = cv::Mat::zeros(total_mask.rows, total_mask.cols, CV_8UC1);
    // }
    ...
    for (int i=0; i<total_mask.rows; i++)
    {
        ...
        {
            if (total_seam_map.at<uchar>(i,j) != 255) continue;
            if (visited.at<uchar>(i,j) == 1) continue;
            if (total_mask.at<uchar>(i,j) < 128) continue;
            ...
            cv::Mat cur_seam_mask = cv::Mat::zeros(total_mask.rows, total_mask.cols, CV_8UC1);
            BFS_get_seam_mask(make_int2(i,j), seam_flag, total_mask, total_seam_map, cur_seam_mask, visited);

            // cv::imwrite("visited" + std::to_string(mark_idx) + ".png", visited * 255);

#if 0
            std::cout << "seam_flag: ";
            for (int k=0; k<seam_flag.size(); k++)
            {
                std::cout << seam_flag[k] << " ";
            }
            std::cout << std::endl;
#endif
            ...
            if (seam_flag[0])
            {
                if (seam_flag.back()) seam_masks[0] = cur_seam_mask;
                else if (seam_flag[1]) seam_masks[1] = cur_seam_mask;
            }else{
...
//        cv::imwrite("seam_mask_" + std::to_string(i) + ".png", seam_masks_cpu[i]);
//        printf("resize %d ..\n", i);
        cv::resize(seam_masks_cpu[i], mask, 
                cv::Size(seam_masks_cpu[i].cols*scale, seam_masks_cpu[i].rows*scale));
//        printf("resize %d done\n", i);
        cudaMemcpy(seam_masks[i], mask.ptr<uchar>(0),
                sizeof(uchar)*seam_masks_cpu[i].rows*seam_masks_cpu[i].cols*scale*scale, cudaMemcpyHostToDevice);
    }
}
#endif 