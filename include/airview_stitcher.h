...
struct BBox
{
    ...
    bool get_remapped_points(const cv::Mat &H, float3 output[4]){
        ...
        float temp1 = -1.0f * H.at<float>(2,0) * x_min - 1.0f * H.at<float>(2,2);
        float temp2 = -1.0f * H.at<float>(2,0) * x_max - 1.0f * H.at<float>(2,2);

        if (H.at<float>(2,1) > 0)
        {
            y_min = min(temp1 / H.at<float>(2,1), temp2 / H.at<float>(2,1)) - 1.0f;
        }else if (H.at<float>(2,1) < 0)
        {
            y_min = max(temp1 / H.at<float>(2,1), temp2 / H.at<float>(2,1)) + 1.0f;
        }else{
            if (temp1 <= 0 || temp2 <= 0) return false;
        }

        float3 p[4] = {
                make_float3(x_min, y_min, 1),
                make_float3(x_max, y_min, 1),
                make_float3(x_max, y_max, 1),
                make_float3(x_min, y_max, 1)};
...
//            printf("get_remapped_points()::i %d point p (%f, %f, %f) warped to (%f, %f, %f)\n",i, p[i].x, p[i].y, p[i].z, warped_p.x, warped_p.y, warped_p.z);
...
//            printf("remap()::i %d point p (%f, %f, %f) warped to (%f, %f, %f)\n",i, p[i].x, p[i].y, p[i].z, warped_p.x, warped_p.y, warped_p.z);

            warped_ps[i] = warped_p;
        }
        cudaMalloc((void**)&data_ptr, sizeof(float3)*4);
        cudaMemcpy(data_ptr, warped_ps, sizeof(float3)*4, cudaMemcpyHostToDevice);
...
public:
    AirViewStitcher(int num_view, int src_height, int src_width, int tgt_height, int tgt_width, int down_scale_seam);
    ~AirViewStitcher();
    void setIcon(std::string path, float size_ratio);
    void init(std::vector<cv::Mat> input_mask, std::vector<cv::Mat> input_H);
    void feed(std::vector<cv::Mat> input_img, std::vector<std::vector<BBox>> bboxs);
